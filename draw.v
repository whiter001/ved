			...
			if current_is_ml_comment {
				if end_comment_pos != -1 { // The multi-line comment ends on this line.
					...
					if normal_part.len > 0 {
						...
						normal_part_x := line_x + ved.text_width_tabs(comment_part)
				...
				if start_comment_pos != -1
					...
					comment_part_x := line_x + ved.text_width_tabs(normal_part)
...
fn prev_utf8_boundary(s string, idx int) int {
	mut i := idx
	if i <= 0 {
		return 0
	}
	...
	for i > 0 && i < s.len && (s[i] & 0xc0) == 0x80 {
		i--
	}
	return i
}

fn (ved &Ved) x_of_byte_idx(line string, byte_idx int) int {
	mut idx := byte_idx
	if idx <= 0 {
		return 0
	}
	...
	idx = prev_utf8_boundary(line, idx)
	mut w := 0
	mut last := 0
	mut i := 0
	for i < idx {
		if line[i] == `\t` {
			if i > last {
				w += ved.gg.text_width(line[last..i])
			}
			w += ved.cfg.tab_size * ved.cfg.char_width
			i++
			last = i
			continue
		}
	...
	if idx > last {
		w += ved.gg.text_width(line[last..idx])
	}
	return w
}

fn (ved &Ved) text_width_tabs(s string) int {
	return ved.x_of_byte_idx(s, s.len)
}
...
fn (ved &Ved) calc_cursor_x() int {
	...
	split_width := ved.split_width()
	line_x := split_width * (ved.cur_split - from) + ved.view.padding_left + 10
	return line_x + ved.x_of_byte_idx(line, ved.view.x)
}
