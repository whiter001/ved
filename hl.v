...
fn (mut ved Ved) draw_text_line_standard_syntax(x int, y int, line string, ext string) {
	...
	for j, chunk in ved.chunks {
		if chunk.start > pos {
			...
			ved.gg.draw_text(cur_x, y, s, ved.cfg.txt_cfg)
			cur_x += ved.text_width_tabs(s)
		}
		typ := chunk.typ
		cfg := match typ {
		...
		ved.gg.draw_text(cur_x, y, s, cfg)
		cur_x += ved.text_width_tabs(s)
		pos = chunk.end
		if j == ved.chunks.len - 1 && pos < line.len {
			final := line[pos..]
			ved.gg.draw_text(cur_x, y, final, ved.cfg.txt_cfg)
		}
	}
